...
public class FeedbackEmpController {
    ...
    public String showAllFeedback(
        ...
        List<Feedback> feedbacks = new ArrayList<>(); // Khởi tạo mặc định
        ...
        try {
            if (trimmedKeyword != null && !trimmedKeyword.isEmpty()) {
                ...
                List<Feedback> result = feedbackDAO.findPaginated(page, pageSize);
                feedbacks = result != null ? result : new ArrayList<>();
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        int totalPages = (totalFeedbacks > 0) ? (int) Math.ceil((double) totalFeedbacks / pageSize) : 0;
